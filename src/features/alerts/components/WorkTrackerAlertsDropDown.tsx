...
import { useWorkTrackerAlerts } from "../hooks/useWorkTrackerAlerts";
...
export function WorkTrackerAlertsDropDown({ workTrackerUuid }: Props) {
  const [showDismissed, setShowDismissed] = useState(false);
  const { activeAlerts, dismissedAlerts, activeCount, dismiss, remindLater, undismiss } =
    useWorkTrackerAlerts(workTrackerUuid);

  const visibleAlerts = showDismissed ? dismissedAlerts : activeAlerts;

  if (activeCount === 0 && dismissedAlerts.length === 0) return null;

  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          className="relative flex items-center justify-center p-1.5 text-gray-500 hover:text-gray-800 cursor-pointer transition-colors"
          aria-label="Work Tracker Alerts"
        >
      ...
      <PopoverContent
        align="end"
        sideOffset={8}
        className="p-0 w-[500px] max-h-[400px] overflow-hidden flex flex-col shadow-lg z-[2100]"
      >
