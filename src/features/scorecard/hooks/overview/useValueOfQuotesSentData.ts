...
export function useValueOfQuotesSentData() {
  const { allEvents, activeRange, periodStart, currentDay, thisPeriodDays, lastPeriodDays, lastPeriodSameElapsedDayKey, timezone } =
    useScorecardStatsContext();

  const goal = 0;

  const thisFiltered = useMemo(
    () => filterQuotesBookingsEvents(allEvents, filtersForTemplate("value-of-quotes-sent", activeRange, "this", null, periodStart), timezone),
    [allEvents, activeRange, periodStart, timezone],
  );
  const lastFiltered = useMemo(
    () => filterQuotesBookingsEvents(allEvents, filtersForTemplate("value-of-quotes-sent", activeRange, "last", null, periodStart), timezone),
    [allEvents, activeRange, periodStart, timezone],
  );

  const thisPeriodCumulativeByDay = useMemo(
    () => cumulativeEventCentsByDay(thisPeriodDays, thisFiltered, "created_at", "contract_revenue_cents", timezone),
    [thisPeriodDays, thisFiltered, timezone],
  );
  const lastPeriodCumulativeByDay = useMemo(
    () => cumulativeEventCentsByDay(lastPeriodDays, lastFiltered, "created_at", "contract_revenue_cents", timezone),
