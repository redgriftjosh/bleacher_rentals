...
function getNumberOfQuotesSentCumulativeByDay(
  days: string[],
  stats: StatsWithDate[],
  ...
  return useMemo(() => {
    ...
    stats.forEach((stat) => {
      ...
      daily[stat.stat_date] += stat.quotes_sent ?? 0;
    });
...
export function useNumberOfQuotesSentData(accountManagerUuid?: string | null) {
  ...
  const { goal } = useTargets(activeRange, "quotes", accountManagerUuid);
  ...
  const thisPeriodCumulativeByDay = getNumberOfQuotesSentCumulativeByDay(
    thisPeriodDays,
    thisPeriodStats,
  );
  const lastPeriodCumulativeByDay = getNumberOfQuotesSentCumulativeByDay(
    lastPeriodDays,
    lastPeriodStats,
  );
