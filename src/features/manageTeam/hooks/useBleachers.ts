...
/**
 * Hook to fetch all bleachers with their home base information
 */
export function useBleachers(): BleacherOption[] {
  const supabase = useClerkSupabaseClient();
  const [bleachers, setBleachers] = useState<BleacherOption[]>([]);

  useEffect(() => {
    async function fetchBleachers() {
      try {
