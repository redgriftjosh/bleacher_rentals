...
export const SubrentalEventForm = ({ onCancel }: SubrentalEventFormProps) => {
  ...
  const handleCreate = async () => {
    setLoading(true);
    try {
      const state = useSubrentalEventStore.getState();
      await createSubrentalEvent(state);
      onCancel();
    } catch {
      // Error toasts handled inside createSubrentalEvent
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const state = useSubrentalEventStore.getState();
      await updateSubrentalEvent(state);
      onCancel();
    } catch {
      // Error toasts handled inside updateSubrentalEvent
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const state = useSubrentalEventStore.getState();
      if (!state.subrentalEventUuid) return;
      await deleteSubrentalEvent(state.subrentalEventUuid);
      onCancel();
    } catch {
      // Error toasts handled inside deleteSubrentalEvent
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <fieldset disabled={!canEdit}>
        {activeTab === "Core" && <SubrentalCoreTab disabled={!canEdit} />}
      </fieldset>
    </div>
  );
};
