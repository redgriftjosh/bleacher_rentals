...
export function ContactDetailModal({ contact, onClose }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateContact(contact.id, { firstName, lastName, email, phone, notes, companyUuid });
      createSuccessToast(["Contact updated."]);
      setMode("view");
    } catch { /* error shown by updateContact */ } finally { setSaving(false); }
  };
  ...
  const displayCompany = contact?.companyName ?? companies.find((c) => c.id === companyUuid)?.companyName;
