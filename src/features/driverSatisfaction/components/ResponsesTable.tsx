...
type ResponsesTableProps = {
  ...
  /** What the score filter asked for, named in the empty state. */
  filterLabel: string;
  /** The score filter itself, rendered in this card's header. */
  filterControl: ReactNode;
  /** How many answers exist in the period before the score filter narrows them. */
...
export default function ResponsesTable({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      {/* The score filter lives here, not in the page header, because it
          narrows this list alone — see the note on DriverSatisfactionPage. */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-gray-100">
        <div>
          <div className="text-sm font-bold text-darkBlue">Answers</div>
          <div className="text-xs text-gray-500">
            {rows.length === totalInPeriod
              ? `${rows.length} in this period`
              : `${rows.length} of ${totalInPeriod} in this period`}
          </div>
        </div>
        {filterControl}
      </div>
      ...
      {rows.length === 0 ? (
        <div className="p-6 text-gray-500">No answers match “{filterLabel}”.</div>
      ) : (
              ...
              {rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex min-w-14 justify-center rounded-md px-2 py-1 font-bold whitespace-nowrap ${scoreClass(row.score)}`}
                    >
