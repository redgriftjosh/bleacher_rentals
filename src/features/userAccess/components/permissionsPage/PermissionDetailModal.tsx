...
export function PermissionDetailModal({ data, onClose }: PermissionDetailModalProps) {
  ...
  const displayNote = data.note ?? DEFAULT_NOTES[data.level];
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-darkBlue">{ROLE_LABELS[data.role]}</span>
          </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
            <p className="text-sm text-gray-700 leading-relaxed">{displayNote}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
